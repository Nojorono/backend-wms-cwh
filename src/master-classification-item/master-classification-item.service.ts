...
export class MasterClassificationItemService {
  ...
  async create(createMasterClassificationItemDto: CreateMasterClassificationItemDto): Promise<MasterClassificationItem> {
    ...
    if (existingClassificationItem) {
      throw new ConflictException(`Classification Item with Code ${classification_code} already exists`);
    }
  ...
  async update(id: string, updateMasterClassificationItemDto: UpdateMasterClassificationItemDto): Promise<MasterClassificationItem> {
    const classificationItem = await this.findOne(id);
    if (updateMasterClassificationItemDto.classification_code && updateMasterClassificationItemDto.classification_code !== classificationItem.classification_code) {
      const existingClassificationItem = await this.repository.findByCode(updateMasterClassificationItemDto.classification_code);
      if (existingClassificationItem) {
        throw new ConflictException(`Classification Item with Code ${updateMasterClassificationItemDto.classification_code} already exists`);
      }
    }

    const updatedClassificationItem = await this.repository.update(id, updateMasterClassificationItemDto);
