...
export class MasterWarehouseBinService {
  ...
  async create(createMasterWarehouseBinDto: CreateMasterWarehouseBinDto): Promise<MasterWarehouseBin> {
    const barcodeImageUrl = await this.barcodeService.generateAndStoreBarcode({
      bcid: 'code128',
      text: `${createMasterWarehouseBinDto.code}-${createMasterWarehouseBinDto.name}` || '',
      scale: 3,
      height: 100,
      width: 200,
      bucket: 'wms',
      ...
      acl: 'public-read',
      metadata: {
        organization_id: createMasterWarehouseBinDto.organization_id?.toString() || '',
        ...
        warehouse_bin_capacity_pallet: createMasterWarehouseBinDto.capacity_pallet?.toString() || '',
      },
    });
  ...
  async update(id: string, updateMasterWarehouseBinDto: UpdateMasterWarehouseBinDto): Promise<MasterWarehouseBin> {
    ...
    if (updateMasterWarehouseBinDto.code || updateMasterWarehouseBinDto.name ||  updateMasterWarehouseBinDto.capacity_pallet) {
      // delete old barcode image
      if (warehouseBin.barcode_image_url) {
        await this.barcodeService.deleteBarcodeImage(warehouseBin.barcode_image_url);
      }
       const barcodeImageUrl = await this.barcodeService.generateAndStoreBarcode({
        bcid: 'code128',
        text: `${updateMasterWarehouseBinDto.code}-${updateMasterWarehouseBinDto.name}` || '',
        scale: 3,
        height: 100,
        width: 200,
        bucket: 'wms',
        ...
        metadata: {
          ...
          warehouse_bin_description: updateMasterWarehouseBinDto.description || warehouseBin.description || '',
          warehouse_bin_capacity_pallet: updateMasterWarehouseBinDto.capacity_pallet?.toString() || '',
        },
      });
