...
export class MasterIOController {
  ...
  @ApiResponse({ status: 201, description: 'The IO has been successfully created.', type: MasterIO })
  ...
  @ApiResponse({ status: 200, description: 'Return all IOs.', type: [MasterIO] })
  findAll() {
  ...
  @ApiResponse({ status: 200, description: 'The IO has been successfully updated.', type: MasterIO })
  @ApiResponse({ status: 404, description: 'IO not found.' })
  @ApiResponse({ status: 409, description: 'IO with this code already exists.' })
  update(
    @Param('id') id: string,
