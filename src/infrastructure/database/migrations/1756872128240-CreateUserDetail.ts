...
export class CreateUserDetail1756872128240 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`INSERT INTO "m_io" ("id", "organization_id", "organization_name", "operating_unit", "created_at", "updated_at") VALUES (uuid_generate_v4(), 1, 'Default Organization', 'Default Unit', now(), now())`);
        
        const defaultOrg = await queryRunner.query(`SELECT id FROM "m_io" WHERE organization_id = 1 LIMIT 1`);
        ...
        if (defaultOrgId) {
            ...
            for (const user of existingUsers) {
                const userDetailId = await queryRunner.query(`INSERT INTO "user_details" ("id", "user_id", "employee_id", "email", "phone", "organization_id", "created_at", "updated_at") VALUES (uuid_generate_v4(), $1, $2, $3, $4, $5, now(), now()) RETURNING id`, 
                    [user.username, `EMP_${user.username}`, `${user.username}@default.com`, '0000000000', defaultOrgId]);
                
                if (userDetailId[0]?.id) {
                    await queryRunner.query(`UPDATE "users" SET "user_detail_id" = $1 WHERE "id" = $2`, [userDetailId[0].id, user.id]);
                }
            }
        }
        ...
        await queryRunner.query(`ALTER TABLE "users" ADD CONSTRAINT "FK_7fbd789ba2d9f9643ff3be7e7b0" FOREIGN KEY ("user_detail_id") REFERENCES "user_details"("id") ON DELETE RESTRICT ON UPDATE NO ACTION`);
    }
